...
CREATE TABLE IF NOT EXISTS OWNER_INFO (
    ...
    CARE_OF VARCHAR,
    ATTN_NAME VARCHAR,
    OWNED_PROPERTIES VARCHAR ARRAY,
    CONSTRAINT OWNER_INFO_PK PRIMARY KEY (COUNTY, SOURCE_INFO_DATE, OWNER_NAME, STREET_ADDRESS, CITY, STATE));
...
CREATE TABLE IF NOT EXISTS ROLL_INFO_AGG (
    ...
    OWNER_NAME_LAST VARCHAR,
    MA_STREET_ADDRESS_LAST VARCHAR,
    MA_UNIT_NUMBER_LAST VARCHAR,
    MA_CITY_LAST VARCHAR,
    MA_STATE_LAST VARCHAR,
    MA_ZIP_CODE_LAST VARCHAR,
    HOMEOWNERS_EXEMPTION_VALUE_LAST INTEGER,
    ...
    OWNER_MOVE_OUT BOOLEAN,
    LAST_DOC_DATE_CHANGE BOOLEAN,
    FUTURE_SALE BOOLEAN,
    CURRENT_TAX_BILL FLOAT,
    DELINQUENT_TAX_BILL FLOAT,
    INTERESTING_PROPERTY BOOLEAN,
    CONSTRAINT PARCEL_PK PRIMARY KEY (COUNTY, PARCEL_ID, SOURCE_INFO_DATE));

CREATE INDEX INTERESTING_PROPERTY_INDEX ON ROLL_INFO_AGG (INTERESTING_PROPERTY);
...
select TAX_INFO_STATUS.COUNTY, TAX_INFO_STATUS.PARCEL_ID, CURRENT_TAX_BILL, DELINQUENT_TAX_BILL, USE_TYPE,
ADDRESS_STREET_NUM, ADDRESS_STREET_NAME, ADDRESS_UNIT_NUM, ADDRESS_CITY, ADDRESS_ZIP, OWNER_NAME, MA_STREET_ADDRESS,
MA_UNIT_NUMBER, MA_UNIT_NUMBER, MA_CITY, MA_STATE, MA_ZIP_CODE from TAX_INFO_STATUS join ROLL_INFO on
(TAX_INFO_STATUS.COUNTY = ROLL_INFO.COUNTY AND TAX_INFO_STATUS.PARCEL_ID = ROLL_INFO.PARCEL_ID) and (SOURCE_INFO_DATE = '2023-08-01')
where CURRENT_TAX_BILL is not null and DELINQUENT_TAX_BILL is not null
order by CURRENT_TAX_BILL/ DELINQUENT_TAX_BILL;

select TAX_INFO_STATUS.COUNTY, TAX_INFO_STATUS.PARCEL_ID, CURRENT_TAX_BILL, DELINQUENT_TAX_BILL, USE_TYPE,
ADDRESS_STREET_NUM, ADDRESS_STREET_NAME, ADDRESS_UNIT_NUM, ADDRESS_CITY, ADDRESS_ZIP, OWNER_NAME, MA_STREET_ADDRESS,
MA_UNIT_NUMBER, MA_UNIT_NUMBER, MA_CITY, MA_STATE, MA_ZIP_CODE from TAX_INFO_STATUS join ROLL_INFO on
(TAX_INFO_STATUS.COUNTY = ROLL_INFO.COUNTY AND TAX_INFO_STATUS.PARCEL_ID = ROLL_INFO.PARCEL_ID)
where CURRENT_TAX_BILL is not null and DELINQUENT_TAX_BILL is not null and TAX_INFO_STATUS.COUNTY = 'SACRAMENTO';


select PARCEL_ID,ADDRESS_STREET_NUM, ADDRESS_STREET_NAME, ADDRESS_CITY, ADDRESS_ZIP ,
OWNER_NAME_LAST, MA_STREET_ADDRESS_LAST, MA_UNIT_NUMBER_LAST,
MA_CITY_LAST, MA_STATE_LAST, MA_ZIP_CODE_LAST, OWNER_NAME_LIST, MA_STREET_ADDRESS_LIST
from ROLL_INFO_AGG where COUNTY = 'SACRAMENTO' and SOURCE_INFO_DATE = '2023' and MA_STREET_ADDRESS_CHANGE = True and
OWNER_NAME_CHANGE = False and USE_TYPE in ('COMMERCIAL_RESIDENTIAL', 'RESIDENTIAL_MULTI_SF', 'RESIDENTIAL_SF') and
HOMEOWNERS_EXEMPTION_VALUE_LAST = 7000;
